...
// Version read from package.json (see version.ts) — never a literal; a
// hardcoded '0.1.0' here once survived five releases unnoticed. `name` stays
// 'mcp-mailman' deliberately — it's the MCP protocol identifier hosts may key
// state on, not a user-facing command name.
const server = new Server({ name: 'mcp-mailman', version: getPackageVersion() }, { capabilities: { tools: {} } });
