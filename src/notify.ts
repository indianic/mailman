...
/**
 * Best-effort native desktop notification, controlled by the
 * `desktopNotifications` setting (on by default; disable with
 * `mailman settings set desktopNotifications false`). Never throws and never
 ...
 * environment (never interpolated into a script string) so it can't break
 * out of, or inject into, the AppleScript/shell command.
 */
...
function dispatch(title: string, body: string): void {
  if (process.platform === 'darwin') {
    ...
    // touches the AppleScript source — no escaping, no injection. The
    // notification is attributed to Script Editor's icon (a macOS limitation
    // of `display notification` — the only way to rebrand it is to ship a
    // signed .app, which isn't worth the complexity here).
    execFile(
      'osascript',
      [
        '-e',
